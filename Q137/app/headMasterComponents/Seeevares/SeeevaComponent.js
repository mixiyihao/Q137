...
import SeeevaItem from './SeeevaItem.js';
import $ from 'jquery';
        ...
        return (
            <div className="classEvaluation_wrap_box">
                ...
                {this.props.list.map((value, index) => {
                    return <SeeevaItem key={index} index={index} value={value}
                                       ObjComponent={this.state.ObjComponent}
                    />
                })}
                <div className="classEvaluation_wrap_textarea">
                    <span>5.其他评价：</span>
                    <textarea style={SeeevaCtextarea} onChange={this.onTextareaChange.bind(this)}
                              value={this.state.detail} className="SeeevaCtextarea" readOnly disabled></textarea>
                </div>
            </div>
        )
    }
