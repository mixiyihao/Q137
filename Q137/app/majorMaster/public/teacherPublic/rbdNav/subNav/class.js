...
export default class Classes extends React.Component {
    ...
    componentWillMount() {
        ...
        if (arrClass != null && arrClass.length > 0) {
            ...
            var id = arrClass[0].id
            var len = arrClass.length;
            for (var i = 0; i < len; i++) {
                if (id == arrClass[i].id) {
                    this.setState({
                        majorNames: arrClass[i].name,
                    })
                    break;
                }
            }
    ...
    // compoentWillReceiveProps(props) {
    //     var arrClass = props.DataTab.majors;
    //     console.log(arrClass)
    // }
            ...
            <div className="rbd-class clearfix rbdItem">
                ...
                <div className="scClass rbd-f clearfix" id="scClass">
                    <div className="editMajor">
                        <span onClick={this.addCourse.bind(this)} className="editHighLight">
                            <i className="iconfont icon-tianjia"></i>
                            <span className="activeItem">新增课程</span>

                        </span>
                        <span onClick={this.editCourse.bind(this)} className={this.state.courseIdItem == '' ? 'editDark' : 'editHighLight'}>
                            <i className="iconfont icon-bianji"></i>
                            <span className="activeItem">编辑课程</span>

                        </span>
                        <span onClick={this.deleCourse.bind(this)} className={this.state.courseIdItem == '' ? 'editDark' : 'editHighLight'}>
                            <i className="iconfont icon-SHANCHU-"></i>
                            <span className="activeItem">删除课程</span>
                        </span>
                        <i className="iconfont icon-shuaxin responseData" onClick={this.responseData.bind(this)}></i>
                    </div>
                    {this.state.termsArr}
                </div>
                <EditCourse
                    ...
                    courseid={this.state.courseIdItem}
                    type={this.state.type}
                    responseData={this.responseData.bind(this)}
                />
                <BombBox
            ...
            for (var i = 0; i < len; i++) {
                list.push(
                    <span
                        ...
                        title={arr[i].name}
                    >
                        <RideoItem vals={arr[i].id} items={this.state.markIt} clickHandle={this.changeCourseId.bind(this)} />
                        {/*
                            // 原有
                            <input type="radio" name="course" className="radioItem" data-id={arr[i].id} onClick={this.changeCourseId.bind(this)} />
                        */}
                        <i data-id={arr[i].id} onClick={this.linkTo.bind(this, arr[i].id, 0)}>{arr[i].name}</i></span>
                )
            }
        }
        return list
    ...
    */
    ...
    responseData() {
        $.llsajax({
            ...
            success: data => {
                ...
                this.createTerm(arrClass[key].termcount, arrClass[key].id, arrClass[key].courseList)
                this.setState({
                    data: arrClass,
                    courseIdItem: '',
    ...
    enterClick() {
        ...
        var ci = this.state.courseIdItem;
        var mi = this.state.majorid;
        $.llsajax({
