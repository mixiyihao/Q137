...
export default class exclusiveinterview extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            Item:5,
    ...
    zhuanti_li() {
        if(this.props.NewsHot.length!=0){
    ...
    Exclus() {
        // 这个是上面的标签 类似“PHP” <b
        // style={{background:this.state.ImgData[key].ImgItem[0].bg,color:this.state.ImgD
        // ata[key].ImgItem[0].color}}>{this.state.ImgData[key].ImgItem[0].flag}</b>
    ...
    render() {
        let amount=this.state.Item*(this.props.NewsPage!=null?this.props.NewsPage:1);
   
        let NewsCount=[]; 
            if(this.props.NewsCount){
              NewsCount=this.props.NewsCount;
            }
        let NewsHot=[];
            if(this.props.NewsHot){
              NewsHot=this.props.NewsHot;
            }
        let sprostoreStyle={
            width:sessionStorage.getItem("userJudge")=="S"?"1050px":"1080px",
            margin:"0 auto",
            display:NewsCount.length==0&&NewsHot.length==0?"block":"none"
        }
        return (
            <div>
                <div className="FirstPagekecheng ">
                    <div className="FirstPagekechengLeft">
                        ...
                        <ul>
                         ...
                         <div className="viewMore" style={{display:NewsCount.length==0?"none":"block"}}>
                                <div className="viewLine"></div>
                                <span className={Number(amount<this.props.NewsCount)?"info2":"info"} 
                                    onClick={Number(amount<this.props.NewsCount)?this.props.Newsajax.bind(this,this.props.NewsPage+1):null}>
                                    {Number(amount<this.props.NewsCount)?"加载更多":"没有更多了"}
                                </span>  
                            </div>
                             <div style={{display:NewsCount.length==0?"block":"none"}} className="spro_store">
                    ...
                    <div className="FirstPageInfo2">
                        ...
                        <ul>
                         ...
                         <div style={{display:NewsHot.length==0?"block":"none"}} className="spro_store">
                                   <div className="spro_liststore">
                                      <div className="spro_storeimage">
                                         <i className="spro_storeimagespan">
                                             {"暂无数据"}
