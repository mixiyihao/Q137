            ...
            <div className="spro_questionBody" onClick={this.finalEXAllclose.bind(this)}>
                <div className="spro_thQuBody">
                    <div className="spro_questitle">
                        ...
                        <a href="javascript:;" style={showUpload} className="h-addexamMany1 button commonButton" onClick={this.importMany.bind(this)}><i className="iconfont icon-daoruchengji"></i>导入试卷</a>
                        {
                            this.state.userJudge == "MM" ?
                                <Link to={{ pathname: "/createFinalPaper", query: { id: '' } }} className="button commonButton h-addexamMany2"><i className="iconfont icon-tianjiadaoshijuan"></i>创建期末试卷</Link>
                                :
                                null
                        }
                    </div>
