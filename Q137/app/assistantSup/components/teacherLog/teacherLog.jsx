...
export default class teacherLog extends Component {
    ...
    componentWillMount() {
        ...
        if(sessionStorage.getItem('userJudge')=="EM"||sessionStorage.getItem('userJudge')=="PM"){
            ...
            index = Base64.decode(hash.split('index=')[1].split('&')[0]);
        }else{
            index = Base64.decode(hash.split('index=')[1]);
        }
        // console.log(id)
        // console.log(index)
        this.setState({
        ...
        if(sessionStorage.getItem('userJudge')=="EM"||sessionStorage.getItem('userJudge')=="PM"){
            ...
            if(hash.indexOf('ty=')!=-1){
                type = hash.split('ty=')[1]
            }
        }
        this.setState({
            ty:type,
    ...
    jumpType(type, obj) {
        ...
        switch (type) {
            case '131':
                hashHistory.push('/previewtestpaper?id='+Base64.encodeURI(obj.describe)+'&F=f')
                break;
            case '132':
                hashHistory.push('/previewtestpaper?id='+Base64.encodeURI(obj.describe)+'&F=q')
                break;
            case '14':
    ...
    tansOptionHandle(obj) {
        ...
        if (obj.describe.indexOf('-') > 0) {
            ...
            var f = type == 1 ? 'f' : 'q'
            hashHistory.push('/previewtestpaper?id=' + Base64.encodeURI(obj.describe.split('-')[0]) + '&zf=f')
        }
    }
    findTestRes(obj) {
        if (obj.type == 132) {
            hashHistory.push('/finalEXanalyze?id='+Base64.encodeURI(obj.describe)+'&F=q')
        } else {
            hashHistory.push('/finalEXanalyze?id='+Base64.encodeURI(obj.describe)+'&F=f')
        }
    }
}