...
import ListKid from '../../teacherComponents/tfinalpubliclist/tfinalpubliclistkid.js';
...
export default class teacherfinallist extends React.Component{
    constructor(){
        super();
        this.state={
           //专业数据
 ...
 handleTypeTabmajor(event) {
  ...
  //处理e.target.value的值 同步setState 到伪select 框
   ...
   limajor(){
    return this.state.majors.map((Sprovalue,key)=>{
      return (
        <li key={key}  data-key={key} onClick={this.handleTypeTabmajor.bind(this)} data-flag="major" data-id={Sprovalue.id} title={Sprovalue.name}>{Sprovalue.name}</li>
      )
    })
  ...
  liCou(){
    ...
    return this.state.SproCourseData.map((Sprovalue,key)=>{
         return(
            <li key={key}  data-key={key} onClick={this.handleTypeTabmajor.bind(this)} data-flag="Cou" data-id={Sprovalue.id} title={Sprovalue.name}>{Sprovalue.name}</li>
         )
     })
    }
    else{
    ...
    finallisthead(){
        ...
        return(
            <ul>
                ...
                <li style={{width:listConfig[8]+"%"}} className="finallistcaozuo">操作</li>
            </ul>
        )
    }
    showNext(){
                ...
                />
                    <div className="spro_questionBody" onClick={this.AllClose.bind(this)}>
                        ...
                        <div className="spro_questitle">
                            ...
                            {/* <a className="spro_publicback" href="javascript:history.go(-1)">返回
                            ...
                            <div className="finallisthead">
                              <div className="flkid">
                                    ...
                                    <div className="sprosatPublicleftselect flkidinnerdivthr">批改状态 :
                                        <span onClick={this.ChooseStatepigai.bind(this)} style={spanStylepigai}>{this.state.Spropigainame}</span>
                                            <ul style={ulStylepigai} className="SprothQuestion">
                                              <li title="全部"   data-key="-1"  data-flag="pigai" onClick={this.handleTypeTabpigai.bind(this)}>全部</li>
                                              <li title="未批改" data-key="0"  data-flag="pigai" onClick={this.handleTypeTabpigai.bind(this)}>未批改</li>
                                              <li title="已批改" data-key="1"  data-flag="pigai" onClick={this.handleTypeTabpigai.bind(this)}>已批改</li>
                                            </ul>
                                    </div>
                              </div>
                              <div className="flkid ">
                                  <div className="flkidinner_ser">搜索考试 :
                                        <input  onChange={this.Changeexamname.bind(this)} type="text"
                                        placeholder="按考试名称进行检索"/>
                                        <span onClick={this.Clickexamname.bind(this)}>搜索</span>
                                  </div>
                              </div>
