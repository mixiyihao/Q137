...
import S_previewtestItem2 from './paperAnalyItem2.js';
    ...
    $.llsajax({
      ...
      success: bodyques => {
        ...
        this.state.data.map((index, key) => {
          ...
          if (index.type == 1) {
            List0.push(index);
            this.state.drawdata.map((dindex,dkey)=>{
                if(index.id==dindex.question_id){
                  drawList0.push(dindex);
                }
            })
      ...
      <div >
         <div style={classDatastyle.spro_exam1200auto} ref="myRef">
           <div className="spre_danxuan" style={exList0style}> <h2> 一、 单选题 </h2><span>(共{this.state.exList0.length}道,每题{this.state.score0}分)</span> </div>
              ...
              <div className="spre_duoxuan" style={exList2style}> <h2> 三、 简答题 </h2><span>(共{this.state.exList2.length}道,每题{this.state.score2}分)</span> </div>
                {this.state.exList2.map((index, i) => {
                    return <S_previewtestItem2 key={i} { ...index} zcindex={i} { ...this.props} exLength={this.state.exLength} Item={this.state.exList0.length+this.state.exList1.length} 
                 />
                })
                }
