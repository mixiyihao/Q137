...
export default class quizzexamBody extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            SproCoursname: "",
            SproCoursid: "",
            laiys: [{ lid: 0, name: "公有测验" }, { lid: 1, name: "私有测验" }]
        }
    ...
    lilaiy() {
        return this.state.laiys.map((Sprovalue, key) => {
            return (
                <li key={key} data-key={key} data-ajaxflag={Sprovalue.lid} onClick={this.handleTypeTablaiy.bind(this)} title={Sprovalue.name}>{Sprovalue.name}</li>
            )
        })
    }
    limajor() {
        return this.state.majors.map((Sprovalue, key) => {
            return (
                <li key={key} data-key={key} data-majorid={Sprovalue.id} onClick={this.handleTypeTabmajor.bind(this)} data-flag="major" title={Sprovalue.name}>{Sprovalue.name}</li>
            )
        })
    ...
    liCou() {
        if (this.state.SproCourseData.length != 0) {
            return this.state.SproCourseData.map((Sprovalue, key) => {
                return (
                    <li key={key} data-key={key} data-courseid={Sprovalue.id} onClick={this.handleTypeTabmajor.bind(this)} data-flag="Cou" title={Sprovalue.name}>{Sprovalue.name}</li>
                )
            })
        }
        else {
    ...
    Finallisthtmltitle(EXflag) {
        return (
            <div className="spro_thQuheadqui">
                ...
                <div className="spro_titlaiyquizz" >
                    来源
                        </div>
                ...
                <div className="spro_titcaozquizz">
                    操作
                        </div>
            </div>
        )
            ...
            <div className="spro_questionBody" onClick={this.finalEXAllclose.bind(this)}>
                <div className="spro_thQuBody">
                    <div className="spro_questitle">
                        ...
                        <div><Link className="commonButton button quizzceyan" to={{ pathname: "/createTestPaper", query: { id: "" } }} onMouseDown={this.Onceback.bind(this)}>创建测验试卷</Link></div>
                    </div>
                    ...
                    <div className="spro_selebody_finalq">
                        <div className="spro_thselection spro_select_quizzExam">
                            <div className="sprosatPublicleftselect_quizz sele_finlai" >来源 :
                                      <span onClick={this.ChooseStatelaiy.bind(this)} style={spanStylelaiy}>{this.state.sproVilaiy}</span>
                                <ul style={ulStylelaiy} className="SprothQuestion">
                                    <li title="全部" data-key="allL" data-ajaxflag="2" onClick={this.handleTypeTablaiy.bind(this)}>全部</li>
                                    {this.lilaiy()}
                                </ul>
                            </div>
                            <div className="sprosatPublicleftselect_quizz sele_finmar">课程 :
                                ...
                                <ul style={ulStyleCou} className="SprothQuestion">
                                    <li title="全部课程" data-courseid="" data-key="allC" data-flag="Cou" onClick={this.handleTypeTabmajor.bind(this)}>全部课程</li>
                                    {this.liCou()}
                                </ul>
                            </div>
                            <div className="sprosatPublicleftselect_quizz  sele_fincou" >专业 :
                                ...
                                <ul style={ulStylemajor} className="SprothQuestion">
                                    <li title="全部专业" data-majorid="" data-key="allM" data-flag="major" onClick={this.handleTypeTabmajor.bind(this)}>全部专业</li>
                                    {this.limajor()}
                                </ul>
                            </div>
                        ...
                        <div className="spro_quizzsea">
                            ...
                            <input type="text" placeholder="按名称检索" id="wocao" ref="sproinputteastore" onChange={this.changeHandle.bind(this)} value={this.state.changeValue} />
                    ...
                    <div style={spro_noinfos} className="spro_page" >
                        <div className="spro_pageinner">
                            <div className="exspro_styleteadivfinal">
                                ...
                                <span className="exspro_styletealistspanfinal">第<b className="exspro_styletealistbfinal" style={styleS}>{this.state.page}</b>页</span>
                            </div>
                ...
                <div className="spro_delete">
                    <div className="spro_deletes">
                        ...
                        <div className="spro_prevbtns">
                            ...
                            <button className="spro_prevbtns2 commonButton button" onClick={this.onDelete.bind(this, "for", "for", 3)}>确定</button>
                        </div>
                    </div>
                </div>
            </div>
